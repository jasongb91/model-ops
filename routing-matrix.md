...
- `openrouter/inclusionai/ling-3.0-flash` (fast-track R1 synthesis/drafting)
...
|---|---:|---|---|---|---|
| Scheduled status / morning brief final output | R0 | `openrouter/nvidia/nemotron-3-ultra-550b-a55b:free` | `openrouter/inclusionai/ling-3.0-flash` (shadow-gated) / `openrouter/z-ai/glm-5.3-flash` (shadow-gated) / `openrouter/google/gemini-3.7-flash` | `openrouter/openai/gpt-5.6-sol` | Nemotron 3 Ultra 550B ($0 cost) is primary for morning brief; Ling 3.0 Flash (4.91/5, 0.31s) and GLM 5.3 Flash ($0.075/1M, 5.0/5 on B1) admitted for shadow evaluation; Gemini 3.7 Flash ($0.075/1M) is alternate fast synthesis. GPT-5.6 Sol retained as premium fallback. |
...
| Internal ops status synthesis (B1) | R1 | `openrouter/nvidia/nemotron-3-super-120b-a12b:free` | `openrouter/inclusionai/ling-3.0-flash` / `openrouter/mistralai/mistral-nemo` / `openrouter/z-ai/glm-5.3-flash` / `openrouter/openai/gpt-5.6-luna` | `openrouter/openai/gpt-5.6-sol` | Nemotron 3 Super 120B provides zero API cost; Ling 3.0 Flash (5.0/5, 0.28s), Mistral Nemo (0.79s, clean status synthesis), GLM 5.3 Flash (5.0/5, 0.22s) and GPT-5.6 Luna ($0.20/$1.20) provide high-accuracy low-cost lanes. GPT-5.6 Sol is frontier escalation. |
| CS note drafting / account update first pass (B3) | R1 | `openrouter/google/gemini-3.7-flash` | `openrouter/inclusionai/ling-3.0-flash` / `openrouter/mistralai/mistral-nemo` / `openrouter/z-ai/glm-5.3-flash` / `openrouter/openai/gpt-5.6-luna` | `openrouter/openai/gpt-5.6-sol` | Gemini 3.7 Flash, Ling 3.0 Flash (5.0/5, 0.22s), Mistral Nemo (5.0/5, 100% canonical format, 0.53s), GLM 5.3 Flash, and GPT-5.6 Luna match frontier quality at substantial cost savings. |
| DevOps / distribution audits (B5) | R1/R0 | `openrouter/nvidia/nemotron-3-super-120b-a12b:free` (R1) / `openrouter/openai/gpt-5.6-sol` (R0) | `openrouter/inclusionai/ling-3.0-flash` / `openrouter/mistralai/mistral-nemo` / `openrouter/z-ai/glm-5.3-flash` / `openrouter/google/gemini-3.7-flash` | `openrouter/openai/gpt-5.6-sol` | Nemotron 3 Super is zero-cost R1 default. Ling 3.0 Flash (4.9/5, 0.20s), Mistral Nemo (5.0/5, 0.19s), and GLM 5.3 Flash (5.0/5 on B5) provide ultra-fast audit table synthesis. GPT-5.6 Sol is R0 frontier specialist. |
| Slack triage / categorization / extraction (B2/B4) | R2 | `openrouter/nvidia/nemotron-3-super-120b-a12b:free` or `openrouter/mistralai/mistral-nemo` | `openrouter/z-ai/glm-5.3-flash` / `openrouter/openai/gpt-5.6-luna` / `openrouter/qwen/qwen3.7-flash` | `openrouter/google/gemini-3.7-flash` | Nemotron 3 Super (3.6s, $0), Mistral Nemo (5.0/5 on B2/B4, 0.26-0.33s), GLM 5.3 Flash (0.34s, 5.0/5 JSON), and GPT-5.6 Luna (0.49s) deliver rapid extraction and schema-compliant triage. |
